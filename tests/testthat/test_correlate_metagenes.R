test_that("correlate_metagenes dimensions output", {
  res_run_ica <- deconica::run_fastica (Example_ds, optimal = FALSE, n.comp = 5,
             with.names = TRUE)
   test <-
...
test_that("correlate_metagenes strong threshold", {
  ...
  expect_equal(nrow(test$n),NULL )
  expect_equal(ncol(test$n), NULL)
  expect_equal(length(test$n), length(data.list))
  expect_equal(length(test), 3)
})
